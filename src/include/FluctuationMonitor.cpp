...
FM_STORE_RetCode FluctuationMonitor::Store(std::string level2_src)
{
	...
	std::memcpy(srcBuf, level2_src.c_str(), level2_src.size());
	...
	if (LL_src.price() == LL_src.high())
	{
		std::map<std::string, std::string>::iterator it = HighDataMap.find(LL_src.code());
		if (it == HighDataMap.end())
		{
			char storeBuf[BUFFELENGTH];
			std::memcpy(storeBuf, srcBuf + TOPICHEADSIZE, hdr_src.size() + LL_src.sbeBlockLength());
		...
		else
		{
			...
			std::memcpy(readBuf, it->second.c_str(), hdr_src.size() + LL_src.sbeBlockLength());
	...
	else if (LL_src.price() == LL_src.low())
	{
		std::map<std::string, std::string>::iterator it = LowDataMap.find(LL_src.code());
		if (it == LowDataMap.end())
		{
			char storeBuf[BUFFELENGTH];
			std::memcpy(storeBuf, srcBuf + TOPICHEADSIZE, hdr_src.size() + LL_src.sbeBlockLength());
			std::string m_str_store(storeBuf, hdr_src.size() + LL_src.sbeBlockLength());
	...
	{
		std::vector<std::string>::iterator it = std::find(HoldHighVector.begin(), HoldHighVector.end(), LL_src.code());
		if (it!=HoldHighVector.end()) //target has been found
		{
			HoldHighVector.erase(it);
			PutInfo(SendStr, OPENHIGH_SIGNALID, LL_src.code(), LL_src.date(), LL_src.time(), LL_src.price());
			return FM_OPENHIGH;
		}
		it = std::find(HoldLowVector.begin(), HoldLowVector.end(), LL_src.code());
		if (it != HoldLowVector.end()) //target has been found
		{
			HoldLowVector.erase(it);
