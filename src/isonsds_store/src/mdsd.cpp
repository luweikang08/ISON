...
{
	this->path_ = path;
	bpt::bplus_tree  mdsitree(path_,true);//创建一颗空树
	return 0;
}
int Mdsi::Insert(Key &key,int value)//把键值对应到相应的序号
{
	bpt::bplus_tree  mdsitree(path_);
	//一般很难准确找到对应的key
	if (mdsitree.insert(key.key, value) != 0)
...
{
	bpt::bplus_tree  mdsitree(path_);
	if (mdsitree.search(key.key, value) != 0)
